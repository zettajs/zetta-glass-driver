...
GlassDriver.prototype.onUpdate = function(data) {
  this.data = data;
  this._streams['heading'].emit('data',data.Heading);
  this._streams['accelX'].emit('data',data.LinearAcceleration[0]);
  this._streams['accelY'].emit('data',data.LinearAcceleration[1]);
  this._streams['accelZ'].emit('data',data.LinearAcceleration[2]);
  this._streams['pitch'].emit('data',data.Pitch);
  this._streams['light-level'].emit('data',data.LightLevel);
};
